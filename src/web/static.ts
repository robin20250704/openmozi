...
/** 助手身份（可配置；默认君无忧的客服小君，勿再硬编码上游默认人格「墨子」） */
function getAssistantIdentity(): { name: string; company: string } {
  return {
    name: process.env.ASSISTANT_NAME || "小君",
    company: process.env.ASSISTANT_COMPANY || "君无忧",
  };
}

/** 获取内嵌的 HTML 页面 */
function getEmbeddedHtml(config: MoziConfig): string {
  const { name: assistantName, company: assistantCompany } = getAssistantIdentity();
...
<head>
  ...
  <title>${assistantName} - ${assistantCompany}智能客服</title>
  <style>
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${assistantName} - ${assistantCompany}控制台</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    :root {
      --primary: #4f46e5;
